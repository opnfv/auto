...
ONAP_COMPONENT=${ONAP_COMPONENT:-$ONAP_COMPONENT_MINIMAL}
...
cat $CONFIG_FILE
if ( ! ansible-playbook -i $CONFIG_FILE $KUBESPRAY_OPTIONS cluster.yml ) ; then
...
if [ -n "$ONAP_COMPONENT" ] ; then
    ...
    for COMPONENT in $ONAP_COMPONENT; do
        echo -n " \$COMPONENT"
        sed -i '/^'\${COMPONENT}':$/!b;n;s/enabled: *true/enabled: true/' onap/values.yaml
    done
    echo
else
