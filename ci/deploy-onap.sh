...
sleep 1m            # wait for pods registration
FAILED_PODS_LIMIT=1
MAX_WAIT_PERIODS=500 # over 2 hours
COUNTER=0
PENDING=\$(kubectl get pods --all-namespaces | grep -E '0/|1/2' | wc -l)
while [ \$PENDING -gt \$FAILED_PODS_LIMIT ]; do
  # print header every 20th lines
  if [ \$COUNTER -eq \$((\$COUNTER/20*20)) ] ; then
    printf "%-3s %-29s %-3s/%s\n" "Nr." "Datetime of check" "Err" "Total PODs"
    # number of all PODs is changing during installation, so it has to be recalculated time to time
    ALL_PODS=\$(kubectl get pods --all-namespaces | wc -l)
  fi
  COUNTER=\$((\$COUNTER + 1 ))
