...
do
ssh $SSH_OPTIONS opnfv@"$MACHINE" "bash -s" <<DOCKERINSTALL &
    sudo su
    apt-get update
...
ssh $SSH_OPTIONS opnfv@"$MASTER" "bash -s" <<RANCHERINSTALL &
sudo su
apt install jq -y
...
echo "REGISTER TOKEN"
HOSTREGTOKEN=$(ssh $SSH_OPTIONS opnfv@"$MASTER" cat /tmp/rancher_register_host)
echo "$HOSTREGTOKEN"
...
for MACHINE in $SERVERS;
do
ssh $SSH_OPTIONS opnfv@"$MACHINE" "bash -s" <<REGISTERHOST &
    sudo su
    $HOSTREGTOKEN
    sleep 5
...
ssh $SSH_OPTIONS opnfv@"$MASTER" "bash -s" <<OOMDEPLOY &
sudo su
