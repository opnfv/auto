...
openstack keypair create auto_keys > /root/auto_key.pem
Chmod 600 /root/auto_key.pem
...
# Allow ICMP and SSH protocols to VMs
...
export VM1_IP=`openstack floating ip list --port "" -c "Floating IP Address" -f value`
...
openstack floating ip create floating_net
export VM2_IP=`openstack floating ip list -c "Floating IP Address" -c "Port" -f value | grep None | cut -f1 -d " "`
...
$AUTO_INSTALL_DIR/ci/deploy_onap.sh $VM1_IP $VM2_IP
