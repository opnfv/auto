...
openstack flavor create --ram 64000  --vcpus 8 --disk 45 m1.xlarge
...
openstack server create --flavor m1.xlarge --image xenial --nic net-id=onap_private_network --security-group onap_security_group --key-name auto_keys vm1 --availability-zone nova:cmp001
...
openstack server create --flavor m1.xlarge --image xenial --nic net-id=onap_private_network --security-group onap_security_group --key-name auto_keys vm2 --availability-zone nova:cmp002
sleep 5 # wait for VM init before floating IP can be assigned
