...
# configuration received from FUEL installer (or user)
AUTO_INSTALL_DIR=${AUTO_INSTALL_DIR:-"."}
CMP_COUNT=${CMP_COUNT:-2}                       # number of compute nodes
CMP_MIN_MEM=${CMP_MIN_MEM:-64}                  # RAM of the weakest compute node
CMP_MIN_CPUS=${CMP_MIN_CPUS:-16}                # cpu count of the weakest compute node
CMP_STORAGE_TOTAL=${CMP_STORAGE_TOTAL:-$((50*$CMP_COUNT))} # size of storage for instances
...
PER=90                      # % of compute resources will be consumed by VM
...
echo "openstack flavor create --ram $VM_MEM  --vcpus $VM_CPUS --disk $VM_DISK m1.xlarge"
...
openstack flavor create --ram $VM_MEM  --vcpus $VM_CPUS --disk $VM_DISK m1.xlarge
...
while [ $VM_ITER -le $VM_COUNT ] ; do
    ...
    VM_NAME[$VM_ITER]="vm${VM_ITER}"
    VM_IP[$VM_ITER]=$(openstack floating ip list -c "Floating IP Address" -c "Port" -f value | grep None | cut -f1 -d " " | head -n1)
    openstack server create --flavor m1.xlarge --image xenial --nic net-id=onap_private_network --security-group onap_security_group --key-name auto_keys ${VM_NAME[$VM_ITER]}
